class Game_Player < Game_Character
  ...
  def allow_diagonal_movement
    $PokemonSystem.diagmovement ||= Settings::ALLOW_DIAGONAL_MOVEMENT ? 0 : 1
    return Settings::ALLOW_DIAGONAL_MOVEMENT && $PokemonSystem.diagmovement == 0
  end
  ...
  def update_command_new_8_diag
    ...
    if $PokemonGlobal.forced_movement?
      ...
      if @moved_last_frame ||
         (dir > 0 && dir == @lastdir && (subs >= 0.075 || subs.negative?))
        case dir
...
MenuHandlers.add(:options_menu, :diag_movement, {
  "name"        => _INTL("Movimiento diagonal"),
  "order"       => 150,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Activado"), _INTL("Desactivado")],
  "description" => _INTL("Elige si quieres poder moverte en diagonal.."),
