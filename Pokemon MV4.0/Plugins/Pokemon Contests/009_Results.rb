...
class PokemonContest
	...
	def pbResults
		...
		pbMessage(_INTL("¡Eso es todo para la ronda de talentos!"))
		pbMessage(_INTL("¡Gracias a todos por esta maravillosa exhibición de movimientos de calidad!"))
		pbMessage(_INTL("¡Con esto terminamos todos los juzgamientos ¡Gracias por su excelente esfuerzo!"))
		pbMessage(_INTL("Ahora, todo lo que queda es la proclamación palpitante del ganador."))
		pbMessage(_INTL("¡El juez parece listo para hacer el anuncio!"))
		pbMessage(_INTL("¡Ahora declararemos al ganador!"))
		...
		pbWait(0.2)
		pbMessage(_INTL("¡{1} y {2}, felicidades!",@winningTrainer.name,@winningPokemon.name))
		pbMessage(_INTL("¡Por favor, vengan y acepten su premio!"))
		...
		pbMessage(_INTL("¡Te otorgamos la {1} como premio!", ribbon.name)) if !@playerWin
		if @playerWin
			if @playerPokemon.hasRibbon?(ribbon)
				pbMessage(_INTL("Como {1} ya tiene la \\c[1]{2}\\c[0], ¡Ofrecemos un premio de cortesía!", @playerPokemon.name, ribbon.name))
				pbReceiveItem(GameData::Item.get(ContestSettings::CONTEST_OTHER_PRIZE[@category][@rank]))
			else
				pbMessage(_INTL("¡Te otorgamos una {1} como premio!", ribbon.name))
				@playerPokemon.giveRibbon(ribbon)
				pbMessage(_INTL("\\me[{1}]{2} recibió \\c[1]{3}\\c[0]!\\wtnp[0.9]", "Item get", @playerPokemon.name, ribbon.name))	
			end
...
class PokemonContestResults_Scene
	...
	def pbScene
		pbDisplay(_INTL("¡Primero, los resultados de la ronda de introducción!")) 
		pbShowIntroRoundBars
		pbDisplay(_INTL("¡Ahora, los resultados de la Ronda de Talentos!")) 
		pbShowTalentRoundBars
		pbDisplay(_INTL("Y el ganador es...")) 
		pbShowPlaces
		position = @contest.winningPosition
		trainerName = @contest.winningTrainer.name
		pokemonName = @contest.winningPokemon.name
		pbDisplay(_INTL("Número de entrada {1}: {2} y {3}. ¡Felicitaciones!",position,trainerName,pokemonName))
	end
