...
class Battle::Scene::FightMenu < Battle::Scene::MenuBase
    ...
    if USE_GRAPHICS
     ...
     # addSprite("background", background)
      @buttons = Array.new(Pokemon::MAX_MOVES) do |i|
        ...
        button.src_rect.height = 46 # BUTTON_HEIGHT
     ...
     @infoOverlay = BitmapSprite.new(Graphics.width, Graphics.height, viewport) 
      ...
      @shiftButton.x      = self.x + 450
      @shiftButton.y      = self.y-84 - @shiftBitmap.height
  ...
  def refreshButtonNames
    ...
    @buttons.each_with_index do |button, i|
      ...
      x = button.x + 71 # button.x - self.x + (button.src_rect.width / 2)
      y = button.y + 12 # [328,362,402,442][i] # button.y - self.y + 14
      moveNameBase = Color.new(10, 10, 18)
     ...
     #   moveNameBase = button.bitmap.get_pixel(10, button.src_rect.y + 34)
      ...
      textPos.push([moves[i].name, x, y, :center, moveNameBase, Color.new(185, 185, 185)])
    end
...
class Battle::Scene
  ...
  def pbFightMenu(idxBattler, specialAction = nil)
    ...
    loop do
      ...
      oldIndex = cw.index
      pbUpdate(cw)
      # Actualizar el comando seleccionado
     if Input.trigger?(Input::UP) && (cw.index)!=0
      pbPlayCursorSE()
      cw.index-=1
    elsif Input.trigger?(Input::DOWN) &&  (cw.index)!=3
      pbPlayCursorSE()
      cw.index+=1
    end
