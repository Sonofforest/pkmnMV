...
class PokemonPokedexInfo_Scene
  ...
  def pbDrawMoveList
    ...
    when 3 then title = (@zcrystals) ? _INTL("MOVS. Z") : _INTL("MOVS. DINAMAX")
    when 4 then title = _INTL("MOVS. DINAMAX")
    end
    ...
    4.times do |i|
      ...
      when 0  # Level-up moves
        if moveLevel == 0
          ...
          textpos.push([moveLevel.to_s, (moveLevel < 100) ? 29+19 : 28+15, yPos + 26, :center, base, shadow, :outline])
        end
      #-------------------------------------------------------------------------
      when 1  # TM/Tutor moves
        ...
        when Symbol
          ...
          textpos.push([name[0..1], 29+19, yPos + 4, :center, base, shadow, :outline],
                       [name[2..machine.length], (machine.length < 5) ? 29+19 : 28+19, yPos + 28, :center, base, shadow, :outline])
        else
      ...
      when 3  # Z-Moves
        if @zcrystals
          @zcrystals.each do |item|
            ...
            imagepos.push([_INTL("Graphics/Items/#{item.id}"), 4+19, yPos + 2])
            break		  
          end
        elsif @maxmoves
          icon = Settings::DYNAMAX_GRAPHICS_PATH + "icon_dynamax"
          imagepos.push([icon, + 11+19, yPos + 8])
        end
      #-------------------------------------------------------------------------
      when 4  # Max Moves
        icon = Settings::DYNAMAX_GRAPHICS_PATH + "icon_dynamax"
    ...
    if !@moveCommands.empty?
      ...
      textpos.push(
        ...
        [power <= 1 ? power == 1 ? "???" : "---" : power.to_s, 468+81, 150-4, :center, base2, shadow2],
      ...
      if @sprites["movecmds"].index > 0
        imagepos.push([path + "page_cursor", 178+38, 350+48, 76, 70, 76, 32])
      end
