...
def pbBerryBlender(playerCount=0,specificNames=nil,forceFail=false)
	...
	if !$bag.hasAnyBerry?
		pbMessage(_INTL("¡No tenés ninguna baya!"))
		return false
	end
	ret = false
	pbFadeOutIn {
...
class BerryBlender_Scene
	...
	def pbScene
		...
		if @restarted then @restarted = nil;
			...
			pbMessage(_INTL("Iniciando el Berryblender...")) 
			pbMessage(_INTL("Selecciona una baya de la mochila para usarla en el Berryblender."))
		end
		loop do
			berry = BerryPoffin.pbPickBerryForBlender
			if berry.nil? || berry == 0
				notplay = !pbConfirmMessage(_INTL("¿Querés elegír una baya?"))
				break if notplay
			else
				break
			end
		...
		return true if $bag.hasAnyBerry? && pbConfirmMessage(_INTL("Querés hacer más Pokecubos?"))
		return false
	end
...
class SimpleBerryBlender_Scene
	...
	def fade_out
		return unless @fade
		numFrames = Graphics.frame_rate / 20
