...
class PokeblockCase_Scene
	...
	def pbStartScene
		...
		@sprites["up_arrow"] = AnimatedSprite.new("Graphics/UI/up_arrow", 8, 28, 40, 2, @viewport)
		@sprites["up_arrow"].x = 356
		@sprites["up_arrow"].y = 0
		@sprites["up_arrow"].z = 999
		@sprites["up_arrow"].play
		@sprites["up_arrow"].visible = false
		@sprites["down_arrow"] = AnimatedSprite.new("Graphics/UI/down_arrow", 8, 28, 40, 2, @viewport)
		@sprites["down_arrow"].x = 356
		@sprites["down_arrow"].y = 348
		@sprites["down_arrow"].z = 999
	...
	def pbRefreshIndexChanged
		...
		@sprites["up_arrow"].visible   = itemlist.top_row > 0
		@sprites["down_arrow"].visible = itemlist.top_item + itemlist.page_item_max < itemlist.itemCount
...
class PokeblockCase_Screen
	...
	def pbStartScreen
		...
		loop do
			...
			commands[cmdToss = commands.length]   = _INTL("Tirar")
			commands[cmdDebug = commands.length]  = _INTL("Debug") if $DEBUG
			commands[commands.length]             = _INTL("Cancelar")
			# Show commands generated above
			itemname = item.name
			command = @scene.pbShowCommands(_INTL("{1} seleccionado.", itemname), commands)
			if cmdUse >= 0 && command == cmdUse   # Use item
				...
				# if qty > 0
				  ...
				  # end
				# end
				if pbConfirm(_INTL("¡Seguro de tirar {1}?", itemname))
					pbDisplay(_INTL("Tiraste {1}.", itemname))
				...
				loop do
				  ...
				  when 0
					flavor = item.flavor
					fNames = ["Picante", "Seco", "Dulce", "Amargo", "Agrio"]
...
class PokeblockCondition_Scene
	...
	def pbCreateConditionBars
		pkmn = @party[@index]
		arr = ["Carisma", "Belleza", "Dulzura", "Inteligencia", "Dureza", "Brillo"]
...
class PokeblockCondition_Screen
	...
	def pbStartScreen(pokeblock)
		...
		loop do
			...
			commands = []
			# Generate command list
			commands[cmdUse = commands.length]    = _INTL("Si")
			commands[commands.length]             = _INTL("No")
			pkmnname = pkmn.name
			blockname = @block.name
			command = @scene.pbShowCommands(_INTL("Alimentar a {1} con {2}?", pkmnname, blockname), commands)
			if cmdUse >= 0 && command == cmdUse   # Use item
				pbFadeOutIn {
...
class PokeblockEat_Scene
	...
	def pbScene
		...
		# Cry
		@pokemon.play_cry
		pbMessage(_INTL("{1} comió el {2}!",@pokemon.name,@pokeblock.name))
...
class PokeblockKit_Screen
	...
	def pbStartScreen
		# Get all commands
		command_list = [_INTL("Dar Pokéblocks"),_INTL("Hacer Pokéblocks"),_INTL("Ver Condisión"),_INTL("Cerrar")]
...
class MultiBerrySelectionScreen
  ...
  def pbStartScreen(proc = nil)
    ...
    loop do
	  if ready
		if @scene.pbConfirm(_INTL("¿Licuar estas bayas?"))
			break
		elsif @scene.pbConfirm(_INTL("¿Dejar de licuar?"))
	  ...
	  if !item 
		if @scene.pbConfirm(@scene.selectedBerries.length == 1 ? _INTL("¿Licuar esta baya?") : _INTL("¿Licuar estas bayas?")) 
	  ...
	  next pbDisplay(_INTL("No tenés más de estos para arrojar.")) if qty - @scene.selectedBerries.count(item) <= 0
      cmdUse      = -1
      commands = []
      # Generate command list
      commands[cmdUse = commands.length]    = _INTL("Arrojar")
      commands[commands.length]             = _INTL("Cancelar")
      # Show commands generated above
      itemname = itm.name
      command = @scene.pbShowCommands(_INTL("Arrojar {1} en la licuadora?", itemname), commands) if !ready
      if cmdUse >= 0 && command == cmdUse   # Use item
