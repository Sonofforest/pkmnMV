...
if Item_RockSmash[:active]
  ...
  def pbRockSmash
    if !pbCanUseItem(Item_RockSmash)
      ...
      pbMessage(_INTL("Es una roca agrietada pero un {1} puede ser capaz de romperla.",item_name))
      return false
    end
    item_name = GameData::Item.get(Item_RockSmash[:internal_name]).name
    if pbConfirmMessage(_INTL("Esta roca parece frágil. ¿Te gustaría usar el {1}?", item_name))
  ...
  ItemHandlers::UseFromBag.add(Item_RockSmash[:internal_name], proc do |item|
    ...
    item_name = GameData::Item.get(Item_RockSmash[:internal_name]).name
    pbMessage(_INTL("¡No hay ninguna razón sensata por la que intentes utilizar un {1} ahora!", item_name))
    next 0
  end)
...
if Item_Cut[:active]
  ...
  def pbCut
    if !pbCanUseItem(Item_Cut)
      pbMessage(_INTL("Este árbol parece que se puede talar."))
      return false
    end
    item_name = GameData::Item.get(Item_Cut[:internal_name]).name
    if pbConfirmMessage(_INTL("¡Parece que este árbol se puede talar! ¿Te gustaría usar la {1}?", item_name))
      $stats.cut_count += 1
      pbMessage(_INTL("{1} usó la {2}!", $player.name, item_name))
      return true
    end
    return false
  end
  ...
  ItemHandlers::UseFromBag.add(Item_Cut[:internal_name], proc do |item|
    ...
    item_name = GameData::Item.get(Item_Cut[:internal_name]).name
    pbMessage(_INTL("¡No hay ninguna razón sensata por la que intentes utilizar la {1} ahora!", item_name))
    next 0
  end)
...
if Item_Strength[:active]
  ...
  def pbStrength
    if !pbCanUseItem(Item_Strength)
      pbMessage(_INTL("Es una gran roca, pero es posible que algo pueda empujarla a un lado."))
      return false
    end
    pbMessage(_INTL("Es una gran roca, pero es posible que algo pueda empujarla a un lado."))
    item_name = GameData::Item.get(Item_Strength[:internal_name]).name
    if pbConfirmMessage(_INTL("¿Te gustaría utilizar la {1}?", item_name))
      pbMessage(_INTL("{1} usó la {2}!", $player.name, item_name))
      pbMessage(_INTL("¡La {1} hizo posible mover rocas!", item_name))
  ...
  ItemHandlers::UseFromBag.add(Item_Strength[:internal_name], proc do |item|
    if $PokemonMap.strengthUsed
      item_name = GameData::Item.get(Item_Strength[:internal_name]).name
      pbMessage(_INTL("¡La {1} ya estaba usada!", item_name))
      next 0
    end
    facingEvent = $game_player.pbFacingEvent
    ...
    item_name = GameData::Item.get(Item_Strength[:internal_name]).name
    pbMessage(_INTL("¡No hay ninguna razón sensata por la que intentes usar la {1} ahora!", item_name))
    next 0
  end)
...
if Item_Surf[:active]
  ...
  def pbSurf
    ...
    if !pbCanUseItem(Item_Surf)
      ...
      pbMessage(_INTL("Aún no tienes ningún equipo para nadar.", item_name))
      return false
    end
    if pbConfirmMessage(_INTL("El agua es de un azul profundo... ¿Te gustaría surfear en ella?"))
  ...
  ItemHandlers::UseFromBag.add(Item_Surf[:internal_name], proc do |item|
    if $PokemonGlobal.surfing
      pbMessage(_INTL("Ya estas navegando."))
      next 0
    end
    if !$game_player.can_ride_vehicle_with_follower?
      pbMessage(_INTL("No se puede usar cuando tienes a alguien contigo."))
      next 0
    end
    if GameData::MapMetadata.exists?($game_map.map_id) &&
    ...
    if !$game_player.pbFacingTerrainTag.can_surf_freely ||
      ...
      item_name = GameData::Item.get(Item_Surf[:internal_name]).name
      pbMessage(_INTL("¡No puedes usar la {1} aquí!", item_name))
      next 0
    end
    next 2
  end)
...
if Item_RockClimb[:active]
  ...
  def aiRockClimb
    if !pbCanUseItem(Item_RockClimb)
      pbMessage(_INTL("La pared es muy rocosa. Se puede escalar con el equipo adecuado."))
      return false
    end
    item_name = GameData::Item.get(Item_RockClimb[:internal_name]).name
    if pbConfirmMessage(_INTL("La pared es muy rocosa. ¿Te gustaría utilizar la {1}?", item_name))
      pbMessage(_INTL("{1} usó la {2}!", $player.name, item_name))
      pbHiddenMoveAnimation(nil)
      case $game_player.direction
      when 8 # Looking up
  ...
  ItemHandlers::UseFromBag.add(Item_RockClimb[:internal_name], proc do |item|
    if !$game_player.pbFacingTerrainTag.can_climb
      pbMessage(_INTL("No estás frente a una pared de roca."))
      next 0
    end
    next 2
  end)
