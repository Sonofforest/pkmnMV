...
if !Item_RockClimb[:active]
  ...
  def fmRockClimb
    ...
    if !pbCanUseMove(Item_RockClimb) || (!movefinder)
      pbMessage(_INTL("La pared es muy rocosa. Se podría escalar con el movimiento correcto."))
      return false
    end
    if pbConfirmMessage(_INTL("La pared es muy rocosa.¿Te gustaría utilizar el {1}", GameData::Move.get(move).name))
      speciesname = (movefinder) ? movefinder.name : $player.name
