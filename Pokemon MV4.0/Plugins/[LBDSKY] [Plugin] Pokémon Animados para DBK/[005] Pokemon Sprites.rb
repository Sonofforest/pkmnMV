...
class PokemonSprite < Sprite
  ...
  def constrict(coords)
    @_iconbitmap.constrict_w = coords[0]
    @_iconbitmap.constrict_h = coords[1]
  end
  ...
  def pbSetShadow(sprite, back = false)
    return if !@_iconbitmap
    self.mirror  = back if Settings::INVERTIR_SOMBRA_JUGADOR
    ...
    self.angle  += ((back && Settings::INVERTIR_SOMBRA_JUGADOR) ? 176 : -2)
  end
