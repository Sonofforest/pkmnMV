...
class Battle::Scene::BattlerShadowSprite < RPG::Sprite
  ...
  def pbSetPosition(sprite)
    ...
    if @substitute
      ...
      self.x      = sprite.x
      self.y      = sprite.y
      self.y     -= sprite.bitmap.height / 4 if (@index.odd? || !Settings::INVERTIR_SOMBRA_JUGADOR)
      self.y     -= 25 if @battler.dynamax? && (@index.odd? || !Settings::INVERTIR_SOMBRA_JUGADOR) && Settings::SHOW_DYNAMAX_SIZE
      self.angle  = sprite.angle
      self.angle -= 2
      self.angle -= 2
      self.angle += 178 if (@index.even? && Settings::INVERTIR_SOMBRA_JUGADOR)
      self.mirror = false if !Settings::INVERTIR_SOMBRA_JUGADOR
