...
class DiegoWTsStarterSelection
  def initialize(pkmn1, pkmn2, pkmn3)
    ...
    @sprites["shadow_1"] = IconSprite.new(56+55, 212, @viewport)
    @sprites["shadow_2"] = IconSprite.new(210+55, 212, @viewport)
    @sprites["shadow_3"] = IconSprite.new(364+55, 212, @viewport)
    for i in 1..3
      if StarterSelSettings::STARTERBG == 1 
    ...
    @sprites["ball_1"] = IconSprite.new(102+55, 188, @viewport)
    @sprites["ball_2"] = IconSprite.new(256+55, 188, @viewport)
    @sprites["ball_3"] = IconSprite.new(410+55, 188, @viewport)
    for i in 1..3
    ...
    @sprites["hlight_1"] = IconSprite.new(68+55, 154, @viewport)
    @sprites["hlight_2"] = IconSprite.new(222+55, 154, @viewport)
    @sprites["hlight_3"] = IconSprite.new(376+55, 154, @viewport)
    for i in 1..3
    ...
    @sprites["pkmn_1"].x = StarterSelSettings::STARTER1X + 320
    @sprites["pkmn_1"].y = StarterSelSettings::STARTER1Y + 148
    @sprites["pkmn_2"].x = StarterSelSettings::STARTER2X + 320
    @sprites["pkmn_2"].y = StarterSelSettings::STARTER2Y + 148
    @sprites["pkmn_3"].x = StarterSelSettings::STARTER3X + 320
  ...
  def pbChoosingScene
    ...
    while @endscene == 0
      ...
      if Input.trigger?(Input::RIGHT) && @select < 3
        ...
        @frame = 0
        pbPlayCursorSE
        @sprites["ball_1"].x = 102+55 if @oldsel == 1
        @sprites["ball_2"].x = 256+55 if @oldsel == 2
        @sprites["ball_3"].x = 410+55 if @oldsel == 3
        @sprites["ball_#{@oldsel}"].angle = 0
        @sprites["hlight_1"].x = 68+55 if @oldsel == 1
        @sprites["hlight_2"].x = 222+55 if @oldsel == 2
        @sprites["hlight_3"].x = 376+55 if @oldsel == 3
        @sprites["shadow_1"].x = 56+55 if @oldsel == 1
        @sprites["shadow_2"].x = 210+55 if @oldsel == 2
        @sprites["shadow_3"].x = 364+55 if @oldsel == 3
        @sprites["select"].x = @x[@select]
        @sprites["select"].y = @y[@select]
        @selframe      = 0
        @ballAnimation = 0
