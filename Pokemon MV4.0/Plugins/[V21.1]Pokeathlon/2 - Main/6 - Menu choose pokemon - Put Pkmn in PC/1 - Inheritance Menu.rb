module Pokeathlon
	class Party_Scene_Athlon < PokemonParty_Scene
		...
		def pbStartScene(party,starthelptext,annotations=nil,multiselect=false, medal=false)
			...
			@sprites["partybg"].bitmap = Bitmap.new("Graphics/UI/Party/bg")
	...
	class PartyScreen_Athlon < PokemonPartyScreen
		...
		def pbPokemonMultipleEntryScreenEx(number, medal)
			annot = []
			statuses = []
			ordinals  = [_INTL("Inelegible"), _INTL("N registrado"), _INTL("Baneado")]
			positions = [_INTL("Primero"), _INTL("Segundo"), _INTL("Tercero"), _INTL("Cuato"),
									 _INTL("Quinto"), _INTL("Sexto"), _INTL("Septimo"), _INTL("Octavo"),
									 _INTL("Noveno"), _INTL("Decimo"), _INTL("Onceavo"), _INTL("Doceavo")]
			for i in 0...Settings::MAX_PARTY_SIZE
			...
			# New
			@scene.pbStartScene(@party,_INTL("Elige tus Pokemon."), annot, true, medal)
			...
			loop do
				...
				@scene.pbSetHelpText(_INTL("Elige tus Pokemon."))
				...
				if (statuses[pkmnid] || 0) == 1
					commands[cmdEntry = commands.length]   = _INTL("Participar")
				elsif (statuses[pkmnid] || 0) > 2
					commands[cmdNoEntry = commands.length] = _INTL("No Participar")
				end
				pkmn = @party[pkmnid]
				commands[commands.length] = _INTL("Cancelar")

				# New
				command = @scene.pbShowCommands(_INTL("¿Qué hago con {1}?", pkmn.name), commands, 0, (@party.index(pkmn))) if pkmn

				if cmdEntry>=0 && command==cmdEntry
					if realorder.size >= number && number > 0
						pbDisplay(_INTL("No se pueden incluir más de {1} Pokémon.", ruleset.number))
					else
