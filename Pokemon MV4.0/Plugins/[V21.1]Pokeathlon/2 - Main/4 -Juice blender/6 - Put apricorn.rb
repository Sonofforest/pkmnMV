module Pokeathlon
	class JuiceBlender
		...
		def choose_feature_0_choose_1
			return if @choose != 1
			arr = []
			@apricorn.each { |i| arr << "#{GameData::Item.get(i).real_name} x #{$bag.quantity(i)}" }
			arr << "Cancel"
			...
			# Put apricorn
				...
				# Delete apricorn
				$bag.remove(@apricorn[ret], 1)
				@apricorn.delete_at(ret) if $bag.quantity(@apricorn[ret]) == 0
				...
				if out
					...
					if !$PokemonGlobal.apricorn_juice_chose_first && $PokemonGlobal.apricorn_juice_first.size == 5
						update_message("¡Ya no le puedes poner más!")
						out = true
					# Not first
					elsif $PokemonGlobal.apricorn_juice_put == MAX_LIMIT_MIXED
						update_message("¡Ya no puedes poner más! Toma tu jugo de albaricoque o camina o monta en bicicleta y tómalo más tarde..")
						out = true
					end
