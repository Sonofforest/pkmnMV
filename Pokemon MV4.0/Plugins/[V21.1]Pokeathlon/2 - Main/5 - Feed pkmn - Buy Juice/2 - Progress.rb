module Pokeathlon
	class FeedJuice
		...
		def create_scene
			super
		p @pkmn.inspect  # <- Añade esto temporalmente
 	  p @pkmn[:name].inspect if @pkmn
  	5.times { |i|
    next unless @pkmn && @pkmn[:name] && @pkmn[:name].respond_to?(:athlon_normal) && @pkmn[:name].athlon_normal
    normal = @pkmn[:name].athlon_normal[i]
		...
		def update_star(speed=nil)
			5.times { |i|
			next unless @pkmn[:name] && @pkmn[:name].respond_to?(:athlon_normal) && @pkmn[:name].athlon_normal
				normal = @pkmn[:name].athlon_normal[i]
