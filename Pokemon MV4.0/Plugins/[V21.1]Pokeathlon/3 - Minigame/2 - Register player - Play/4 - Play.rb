module Pokeathlon
	class Play_Athlon
		...
		def play
			...
			# Presentation
			mess("Bienvenidos al pokeathlon!")
			mess("¡El evento del día es la Exhibición de #{namecourse}!")
			mess("¿Quién brillará más hoy? ¡Conozcamos a los competidores!")
			# Show pokemon
			fade_in
			...
			# Player
			mess("Primero") { update_animated_pokemon(@store[:species]) }
			...
			mess("Equipo #{@store[:team][0]}: #{teampkmn}") { update_animated_pokemon(@store[:species]) }
			...
			3.times { |i|
				...
				mess("Equipo #{@store[:team][3+3*i]}: #{teampkmn}") { update_animated_pokemon(@store[:species]) }
			...
			mess("¡Muy bien, estamos listos! ¡Apunta a la cima! Vamos.") { update_animated_pokemon(@store[:species]) }
			...
			mess("¡Buen trabajo, atletas!") { update_animated_pokemon(@store[:species]) }
			mess("¿Qué equipo luchó más? ¡Tenemos los resultados!") { update_animated_pokemon(@store[:species]) }
			mess("Pero primero...¿quién rebosa de espíritu de lucha?") { update_animated_pokemon(@store[:species]) }

			# Challenge Bonus
			mess("¡Un bono de desafío para Pokémon y su equipo!") { update_animated_pokemon(@store[:species]) }
			# Increase points
			sum = []
			num = 0
			12.times { |i|
			...
			4.times { |i|
				...
				mess("#{@store[:score][i]} puntos para el equipo #{@store[:team][3*i]}") { update_animated_pokemon(@store[:species]) }
			...
			# No-Miss Bonus
			mess("Continuando, los premios individuales...") { update_animated_pokemon(@store[:species]) }
			count = @store[:miss].count { |mi| mi == 0 } > 0
			if count
				mess("¡El bono que no te puedes perder!") { update_animated_pokemon(@store[:species]) }
				# Increase points
				scorefake = @store[:score].clone
				pos = []
				...
				pos.each { |po|
					...
					mess("Equipo #{@store[:team][po]}'s #{@store[:id][po]}") {
				...
				mess("¡Eso es todo!") { update_animated_pokemon(@store[:species]) }
			else
				mess("Esta vez no había ninguno.") { update_animated_pokemon(@store[:species]) }
			end

			# Leading Score Bonus
			mess("Siguiendo adelante...¡el líder en puntos!") { update_animated_pokemon(@store[:species]) }
			# Increase points
			...
			mess("¡A continuación... el premio al esfuerzo!") { update_animated_pokemon(@store[:species]) }
			mess("El Pokémon que no logró recolectar más puntos...") { update_animated_pokemon(@store[:species]) }
			# Increase points
			max = @store[:miss].max
			scorefake = @store[:score].clone
			pos = []
			...
			# Points (3 events)
			mess("Por último...¡sumaremos los puntos del evento!") { update_animated_pokemon(@store[:species]) }
			arr = ["Evento 1", "Evento 2", "Evento 3"]
			3.times { |i|
			...
			max = @store[:score].max
			index = @store[:score].index(max)
			mess("El ganador con #{max} puntos es...") { update_animated_pokemon(@store[:species]) }
			...
			mess("Equipo #{@store[:team][3*index]}") { update_animated_pokemon(@store[:species]) }

			# Show medal
			w = @sprites["medal"].src_rect.width
			course = ["Velocidad", "Fortaleza", "Precisión", "Resistencia", "Salto"]
			x = course.index(namecourse)
			set_src_xy_sprite("medal", x * w, 0)
			set_visible_sprite("medal", true)

			mess("Equipo #{@store[:team][3*index]} fue premiado con la medalla #{namecourse}") { update_animated_pokemon(@store[:species]) }
			mess("¡Lo hiciste bien en el evento de #{namecourse}!") { update_animated_pokemon(@store[:species]) }

			# Show message
			mess("¡Felicitaciones! ¡Fue una actuación maravillosa!") { update_animated_pokemon(@store[:species]) }
			mess("¡Sigue apuntando a ser el mejor Pokeatleta!") { update_animated_pokemon(@store[:species]) }
