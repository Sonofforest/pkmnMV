...
GameData::SecretBaseSkill.register({
  :id         => :EV_Training,
  :name       => _INTL("hacer algo de ejercicio"),
  :usage_proc => proc {|owner|
    pbMessage(_INTL("{1}: ¿Con qué Pokémon debería hacer ejercicio?",owner.name))
    chosen = 0
    pbFadeOutIn {
    ...
    if evTotal>=Pokemon::EV_LIMIT
      pbMessage(_INTL("{1}: {2} no parece que necesite ningún ejercicio.",owner.name,pkmn.name))
      next false
    elsif pkmn.egg?
      pbMessage(_INTL("{1}:¿Cómo se ejercita un Huevo?",owner.name))
      next false
    end
