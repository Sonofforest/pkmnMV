  ...
  loop do
    command = pbShowCommandsWithHelp(nil,
         [_INTL("Decorar"),
          ...
          _INTL("Tirar"),
          _INTL("Cancelar")],
         [_INTL("Coloque el elemento de decoración seleccionado."),
          _INTL("Guardar la decoración elegida en el PC."),
          _INTL("Desechar las decoraciones no deseadas."),
          _INTL("Regresar al menú anterior.")], -1, command)
    case command
    when 0 
...
class Window_BasePocketsList < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if index == SecretBag.pocket_count
      textpos.push([_INTL("CANCELAR"), rect.x, rect.y, false, self.baseColor, self.shadowColor])
    else
  ...
  def pbChooseItem(continue_selection=false)
    ...
    loop do
      ...
      if decorwindow.item
        ...
        pbShowDescription(_INTL("Regresar al menú anterior."))
      end
...
class SecretBagScreen
  ...
  def pbDecorate(base)
    ...
    loop do
      ...
      if @bag.is_placed?(item[0],item[1])
        ...
        @scene.pbDisplay { 
            pbMessage(_INTL("No se pueden colocar más decoraciones.\nLo máximo que se puede colocar es {1}.", SecretBaseSettings::SECRET_BASE_MAX_DECORATIONS))
          }
      else
  ...
  def pbTossItems
    ...
    loop do
      ...
      if @bag.is_placed?(item[0],item[1])
        @scene.pbDisplay { 
          pbMessage(_INTL("Esta decoración está en uso. No se puede tirar."))
        }
      else
        decor = @bag.pockets[item[0]][item[1]]
        decorname = GameData::SecretBaseDecoration.get(decor[0]).name
        @scene.pbDisplay { 
          if pbConfirmMessage(_INTL("Este {1} será descartado. \ N¿Está bien?",decorname))
            @bag.remove_at_index(item[0],item[1])
            pbMessage(_INTL("El objeto de decoración fue tirado a la basura.."))
          end
        }
      end
      firstloop = false
...
class PlaceDecoration_Scene
  ...
  def pbSelectTile
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        msg = (@item) ? _INTL("¿Cancelar la decoración?") : _INTL("¿Dejar de guardar las decoraciones?")
  ...
  def check_location
    ...
    if @item
      ...
      if cant_place
        pbMessage("No se puede colocar aquí.") { pbUpdate }
      else
        if pbConfirmMessage(_INTL("¿Colocarlo aquí?")) { pbUpdate }
          ret = true
          pbFadeOutIn(99999){
      ...
      if decors.length == 0
        pbMessage("No hay ningún elemento de decoración aquí..") { pbUpdate }
      else
        if pbConfirmMessage(_INTL("¿Devolver esta decoración al PC?")) { pbUpdate }
          pbFadeOutIn(99999){
