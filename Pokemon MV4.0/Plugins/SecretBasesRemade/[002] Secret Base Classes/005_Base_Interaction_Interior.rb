...
def pbSecretBasePCMenu
  ...
  command = 0
  loop do
    choice = pbMessage(_INTL("¿Qué es lo que quieres hacer?"), command_list, -1, nil, command)
    if choice < 0
      pbPlayCloseMenuSE
      break
    end
...
MenuHandlers.add(:secret_base_pc_menu, :pack_up, {
  "name"      => _INTL("Empaquetar"),
  "order"     => 20,
  "effect"    => proc { |menu|
    player_base = $PokemonGlobal.secret_base_list[0]
    base_data = GameData::SecretBase.get(player_base.id)
    pbMessage(_INTL("Todas las decoraciones y muebles de tu Base Secreta serán devueltos a tu PC.\\1"))
    if pbConfirmMessage(_INTL("¿Está bien?"))
      # Pack up the base.
      pbFadeOutIn(99999) {
...
def pbSecretBasePCRegisterMenu
  pbMessage(_INTL("{1} arrancó la PC.", $player.name))
  # Get all commands
  command_list = []
  commands = []
...
MenuHandlers.add(:secret_base_pc_register_menu, :register, {
  ...
  "effect"    => proc { |menu|
    ...
    case validity
    when 2
      pbMessage(_INTL("Se pueden registrar hasta 10 ubicaciones."))
      pbMessage(_INTL("Eliminar una ubicación si desea registrar otra ubicación."))
      next false
    when 1
      msg = _INTL("Estos datos ya están registrados.\n¿Desea eliminarlos?")
    when 0
      msg = _INTL("¿Quieres registrar la Base Secreta de\n{1}??",base.owner.name)
    end
    if pbConfirmMessage(msg)
      base.registry_status^=1
      $PokemonGlobal.secret_base_registry = true
      if validity == 1
        pbMessage(_INTL("Los datos no han sido registrados."))
      else
        pbMessage(_INTL("Registro completado."))
      end
    end
    next false
  }
