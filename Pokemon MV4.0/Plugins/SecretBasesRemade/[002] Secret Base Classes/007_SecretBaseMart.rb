  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["itemtextwindow"].text =
        (itemwindow.item) ? @adapter.getDescription(itemwindow.item) : _INTL("Salir.")
      itemwindow.refresh
    end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getMoney < price
        pbDisplayPaused(_INTL("No tenés dinero suficiente."))
        next
      end
      next if !pbConfirm(_INTL("Querés {1}?\nserían ${2}. ¿ok?",
      ...
      if added == quantity
        ...
        pbDisplayPaused(_INTL("¡Gracias!\nLo enviaré a tu PC en casa.")) { pbSEPlay("Mart buy item") }
      else
        added.times do
          if !@adapter.removeItem(item)
            raise _INTL("Fallo al borrar los objetos almacenados")
          end
        end
        pbDisplayPaused(_INTL("El espacio para {1} está lleno.", itemname))
      end
    end
    @scene.pbEndBuyScene
  end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if @adapter.isPlaced?(item[0],item[1])
        pbDisplayPaused(_INTL("Perdón, ese {1} está en uso.\nNo puedo comprarlo.", itemname))
          next
      elsif !@adapter.canSell?(decor[0])
        pbDisplayPaused(_INTL("Oh, no puedo comprar el {1}.", itemname))
        next
      end
      price = @adapter.getPrice(decor[0], true)
      @scene.pbShowMoney
      if pbConfirm(_INTL("Puedo pagar ${1}.\n¿Está bien?", price.to_s_formatted))
...
def pbSecretBaseMart(stock, speech = nil, cansell = false)
  ...
  cmdSell = -1
  cmdQuit = -1
  commands[cmdBuy = commands.length]  = _INTL("Vengo de compras")
  commands[cmdSell = commands.length] = _INTL("I'm here to sell") if cansell
  commands[cmdQuit = commands.length] = _INTL("No, gracias")
  cmd = pbMessage(speech || _INTL("¿Cómo le vá? ¡No dude en comprar!"), commands, cmdQuit + 1)
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      ...
      pbMessage(_INTL("¡Vuelva pronto!"))
      break
    end
    cmd = pbMessage(_INTL("¿Algo más?"), commands, cmdQuit + 1)
  end
