...
class Battle::Scene
  ...
  def pbUpdateBattlerInfo(battler, effects, idxEffect = 0)
    ...
    if battler.hp > 0
      ...
      imagePos.push(["Graphics/UI/Battle/overlay_hp", 86+30, 88-16-8, 0, hpzone * 6, w, 6])
    end
    # Battler status.
    ...
    if battler.pbOwnedByPlayer?
      imagePos.push(
        ...
        [@path + "info_cursor", panelX, 64-16-8, 0, 0, 256, 24],
        [@path + "info_cursor", panelX, 88-16-8, 0, 0, 256, 24]
      )
      textPos.push(
        [_INTL("Hab."), xpos + 272+30, ypos + 44, :center, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("Obj."), xpos + 272+30, ypos + 68, :center, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("{1}", battler.abilityName), xpos + 376+30, ypos + 44, :center, BASE_DARK, SHADOW_DARK],
        [_INTL("{1}", battler.itemName), xpos + 376+30, ypos + 68, :center, BASE_DARK, SHADOW_DARK],
  ...
  def pbAddTypesDisplay(xpos, ypos, battler, poke)
    ...
    displayTypes.each_with_index do |type, i|
      ...
      @enhancedUIOverlay.blt(xpos + 170+14, typeY + (i * 30), typebitmap.bitmap, type_rect)
    end
    #---------------------------------------------------------------------------
    # Draws Tera type.
  ...
  def pbAddEffectsDisplay(xpos, ypos, panelX, effects, idxEffect)
    ...
    imagePos = [
      ...
      [@path + "info_slider_base", panelX + 222+44, ypos + 134]
    ]
    #---------------------------------------------------------------------------
    # Draws the slider.
    #---------------------------------------------------------------------------
    if effects.length > 5
      imagePos.push([@path + "info_slider", panelX + 222+44, ypos + 134, 0, 0, 18, 19]) if idxEffect > 3
      imagePos.push([@path + "info_slider", panelX + 222+44, ypos + 235, 0, 19, 18, 19]) if idxEffect < idxLast - 1
      sliderheight = 82
      ...
      imagePos.push([@path + "info_slider", panelX + 222+44, y, 18, 0, 18, 4])
      i = 0
      while i * 7 < boxheight - 2 - 7
        height = [boxheight - 2 - 7 - (i * 7), 7].min
        offset = y + 2 + (i * 7)
        imagePos.push([@path + "info_slider", panelX + 222+44, offset, 18, 2, 18, height])
        i += 1
      end
      imagePos.push([@path + "info_slider", panelX + 222+44, y + boxheight - 6 - 7, 18, 9, 18, 12])
    end
    #---------------------------------------------------------------------------
    ...
    effects[idxStart..idxEnd].each_with_index do |effect, i|
      ...
      if i == idxDisplay || idxEffect == real_idx
        imagePos.push([@path + "info_cursor", panelX, ypos + 132 + (i * 24), 0, 48, 218, 24])
      else
        imagePos.push([@path + "info_cursor", panelX, ypos + 132 + (i * 24), 0, 24, 218, 24])
      end
    ...
    drawFormattedTextEx(@enhancedUIOverlay, xpos + 246, ypos + 268-4, 310, desc, BASE_LIGHT, SHADOW_LIGHT, 18)
  end
      ...
      key_hash.each do |type, effects|
        effects.each do |effect|
          ...
          when :Protect, :SpikyShield, :BanefulBunker
            name = _INTL("Protegido completamente")
